 ...
 */
function getAllMovieTitles(movieList) {
  if (!movieList.length){
    throw 'Error: No movies found';
  }

  let movieTitles = movieList.map(el => {
    return el.title;
  })
  return movieTitles;
}
 ...
 *  //> false
 */
function checkIfAnyMovieHasRating(movieList, rating = 'G') {
  if (!movieList.length){
    throw 'Error: No movies found';
  }

  let hasRating = movieList.some(el => {
    return el.rated === rating;
  });
  return hasRating;
}
 ...
 */
function findById(movieList, id) {
  if (!movieList.length){
    throw 'Error: No movies found';
  }

  let foundMovie = movieList.find(el => {
    return el.imdbID === id;
  });
 ...
 */
function getAllMoviesReleasedAtOrBeforeYear(movieList, year) {
  if (!movieList.length){
    throw 'Error: No movies found';
  }

  movieList.forEach(el => {
    let releaseDate = el.released;
    let releaseArr = releaseDate.split(' ')
    let releasedYear = releaseArr[releaseArr.length-1]
    el.releasedYear = releasedYear; 
  });

  let filteredMovies = movieList.filter(el => {
    return el.releasedYear <= year;
  });
 ...
 */
function getRottenTomatoesScoreByMovie(movieList) {
  if (!movieList.length){
    throw 'Error: No movies found';
  }
